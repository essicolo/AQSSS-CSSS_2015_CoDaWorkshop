...
**R** is a modular open source statiscitcal programming environment mainly used for data analysis and plotting. It can be used for classical statitics, data mining and machine learning in a large range of fields: ecology, economy, geography, demography, climatology, and... soils!
...
powerRanger <- function(x, power) {
  y <- range(x^power) # range is a builtin function
  return(y)
}
powerRanger(x = seq(from = 2, to = 10, by = 1), power = 2) # x is a vector, seq is a builtin function
```

## Vector and matrices

Vectors and matrices contain a single class:
```{r}
...
Lists are collections of heterogeneous data. They are very useful to store function outputs in loops, like the output of anovas of linear regressions. Data frames are lists with equal number of elements. They are used for handling tables.
...
**R** can be used with only a terminal and a text editor, but GUIs are useful. I highly recommand **RStudio**, an open source interface for **R** developed and supported by an eponym commercial enterprise. It can be downloaded from: http://www.rstudio.com/.
...
Suggested packages (very unexhaustive):
...
The dataset is exempt of any zeros or missing values. Let's **replace small values by zeros**.
...
** Zero spending in alcohol in a budget of a person that stricktly does not drink: is this observation important (any exmaple in soil science)?
...
Now that the dataset is clean, close the simplex.
